...
namespace Logical.Editor.UIElements
{
    public class TwoPaneSplitView : VisualElement
    {
        ...
        public new class UxmlTraits : VisualElement.UxmlTraits
        {
            ...
            UxmlIntAttributeDescription m_MinPaneSize = new UxmlIntAttributeDescription { name = "min-pane-size", defaultValue = 10 };
            ...
            public override void Init(VisualElement ve, IUxmlAttributes bag, CreationContext cc)
            {
                ...
                var minDimension = m_MinPaneSize.GetValueFromBag(bag, cc);
                var orientationStr = m_Orientation.GetValueFromBag(bag, cc);
                var orientation = orientationStr == "horizontal"
                    ? Orientation.Horizontal
                    : Orientation.Vertical;

                ((TwoPaneSplitView)ve).Init(fixedPaneIndex, fixedPaneInitialSize, minDimension, orientation);
            }
        }
        ...
        public TwoPaneSplitView()
        {
            ...
            styleSheets.Add(Resources.Load<StyleSheet>(ResourceAssetPaths.TwoPaneSplitView_StyleSheet));
        ...
        {
            Init(fixedPaneIndex, fixedPaneStartDimension, minDimension, orientation);
        }

        public void Init(int fixedPaneIndex, float fixedPaneInitialDimension, float minDimension, Orientation orientation)
        {
        ...
        void OnPostDisplaySetup(GeometryChangedEvent evt)
        {
            ...
            if(m_queuedSplitPosition != 0)
            {
                m_Resizer.ApplyNewPosition(m_queuedSplitPosition);
                m_queuedSplitPosition = 0;
            }
        }
        ...
        public float SplitPosition { get { return m_Resizer.Position; } }
        private float m_queuedSplitPosition = 0;
        public void SetSplitPosition(float size)
        {
            if (m_Resizer != null)
            {
                m_Resizer.ApplyNewPosition(size);
            }
            else
            {
                m_queuedSplitPosition = size;
            }
        }
        ...
        class SquareResizer : MouseManipulator
        {
            ...
            public void ApplyDelta(float delta)
            {
                ...
                ApplyNewPosition(newDimension);
            }

            public void ApplyNewPosition(float position)
            {
                if (m_Orientation == Orientation.Horizontal)
                {
                    m_Pane.style.width = position;
                    if (m_SplitView.m_FixedPaneIndex == 0)
                        target.style.left = position;
                    else
                        target.style.left = m_SplitView.resolvedStyle.width - position;
                }
                else
                {
                    m_Pane.style.height = position;
                    if (m_SplitView.m_FixedPaneIndex == 0)
                        target.style.top = position;
                    else
                        target.style.top = m_SplitView.resolvedStyle.height - position;
                }
                Position = position;
            }
