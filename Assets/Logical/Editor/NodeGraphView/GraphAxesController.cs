...
namespace Logical.Editor
{
    ...
    public class GraphAxesController : VisualElement
    {
        ...
        public GraphAxesController(NodeGraphView nodeGraphView, CustomContentDragger contentDragger,
            SecondarySelectionDragger secondarySelectionDragger)
        {
            ...
            m_secondarySelectionDragger.OnDragging +=
                RefreshPositions; // TODO: This is a hack to get the axes to update when moving nodes around. But it still doesn't work perfectly.
        }

        ~GraphAxesController()
        {
            if (m_nodeGraphView != null)
            {
        ...
        public void RefreshPositions()
        {
            ...
            Vector2 center_ScreenSpace = new Vector2(min_GraphSpace.x - offset_GraphSpace.x,
                min_GraphSpace.y - offset_GraphSpace.y);
            ...
            Vector2 upperBound = new Vector2(
                Mathf.Ceil((max_ScreenSpace.x + offset_GraphSpace.x - min_GraphSpace.x) / pixelsPerIncrement) *
                pixelsPerIncrement,
                Mathf.Ceil((max_ScreenSpace.y + offset_GraphSpace.y - min_GraphSpace.y) / pixelsPerIncrement) *
                pixelsPerIncrement);
            ...
            for (float i = lowerBound.y; i <= upperBound.y; i += pixelsPerIncrement)
            {
                ...
                GetCoordinateLabel().SetLabel((int)(Mathf.Round((i / zoom) / increment) * increment),
                    new Vector2(Mathf.Max(center_ScreenSpace.x, AXIS_LINE_WIDTH - 1),
                        i - offset_GraphSpace.y + min_GraphSpace.y),
                    (center_ScreenSpace.x < max_ScreenSpace.x)
                        ? CoordinateLabel.Direction.RIGHT
                        : CoordinateLabel.Direction.LEFT);
            }
            ...
            for (float i = lowerBound.x; i <= upperBound.x; i += pixelsPerIncrement)
            {
                ...
                GetCoordinateLabel().SetLabel((int)(Mathf.Round((i / zoom) / increment) * increment),
                    ...
                    (center_ScreenSpace.y != max_ScreenSpace.y)
                        ? CoordinateLabel.Direction.DOWN
                        : CoordinateLabel.Direction.UP);
            }
        }
        ...
        public class GraphAxis : GraphElement
        {
            ...
            {
                SetPosition(new Rect(position - new Vector2(HALF_AXIS_LINE_WIDTH, HALF_AXIS_LINE_WIDTH),
                    GetPosition().size));
            }
